...
class QueueBasedGraphEngine:
    ...
    def __init__(
        ...
        node_executor: Optional[Callable[[Task], NodeRunResult]] = None,
    ):
        self.graph = graph
        ...
        self.task_queue: TaskQueueProtocol[Task] = (
            task_queue if task_queue is not None else queue.Queue()
        )
        ...
        self.node_executor = (
            node_executor if node_executor is not None else self._default_node_executor
        )
    ...
    def _enqueue_task(self, node_id: str) -> None:
        ...
        task = Task(
            node_id=node_id,
            variable_pool=self.runtime_state.variable_pool.model_copy()
        )
    ...
    def _default_node_executor(self, task: Task) -> NodeRunResult:
        ...
        # This would normally instantiate and run the actual node
